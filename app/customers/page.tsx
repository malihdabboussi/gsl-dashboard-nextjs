...
import { createCustomer, getAllCustomers } from "../actions";
...
export default function CustomersPage() {
  ...
  const refs = {
    ...
    billingInformation: {
      address: {
        ...
        lines: [useRef<HTMLInputElement>(null), useRef<HTMLInputElement>(null)],
  ...
  const setCreditDates = (dates: DateRange) => {
    if (dates.from && dates.to) {
      refs.creditLimitDuration.startDate.current.value =
  ...
  const CustomDialog = ({
    ...
    setCreditDates: (date: DateRange) => void;
    refs: Refs;
  }) => {
        ...
        .then((data) => {
          ...
          setPhoneCountries(
            data
              .map((country) => {
                const datum =
                  country.alpha2Code + " +" + country.callingCodes.join(",");
      ...
      <Dialog>
        ...
        <DialogContent className="w-96">
          ...
          <form className="grid gap-4 py-4">
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="startDate">Credit limit duration</Label>

              <div className={"grid gap-2"}>
                <DatePickerWithRange
                  text="Choose the range of dates"
                  onChange={(dates) => {
            ...
            <div className="flex flex-col gap-2">
              ...
              />
              <Input
                ref={refs.billingInformation.address.lines[0]}
                id="addressLine1"
                placeholder="Address line 1"
                className="col-span-3"
              />
              <Input
                ref={refs.billingInformation.address.lines[1]}
                id="addressLine2"
                placeholder="Address line 2"
                className="col-span-3"
              />
              <Input
                ref={refs.billingInformation.address.town}
                id="town"
                placeholder="Town/city"
                className="col-span-3"
              />
              <Input
                ref={refs.billingInformation.address.postalCode}
                id="postalCode"
              ...
              <div className="flex gap-1">
                <Combobox
                  ...
                  setValue={setSelectedPhoneCountry}
                  errorMessage=""
                  className="!w-min"
                />
                <Input
                  ref={refs.billingInformation.phone.phoneNumber}
                  id="phoneNumber"
  ...
  // start
  const getData = async () => {
    // collection: "all" | "top" | "first-time" | "repeat"
    const dataAll: Entity[] = await getAllCustomers();
    const dataTop: Entity[] = await getAllCustomers();
    const dataFirst: Entity[] = await getAllCustomers();
    const dataRepeat: Entity[] = await getAllCustomers();
    setData({
      all: dataAll,
      top: dataTop,
      firstTime: dataFirst,
  ...
  return (
    <BasePage customHead={head} pageId={2}>
